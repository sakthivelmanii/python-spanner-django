...
class DatabaseOperations(BaseDatabaseOperations):
    ...
    def bulk_batch_size(self, fields, objs):
        ...
        objects that can be in a batch based on the max_query_params.
    ...
    def sql_flush(
        ...
        # Sort tables by dependency order (children first) to avoid FK violations.
        # If A references B, A must be deleted before B.
        # Dependency graph: B depends on A (B cannot be deleted until A is deleted).
        from graphlib import TopologicalSorter
        sorter = TopologicalSorter()
        tables_set = set(tables)

        # Add all tables to ensure they are in the graph even if isolated
        for table in tables:
            sorter.add(table)
        ...
        with self.connection.cursor() as cursor:
            for table in tables:
                try:
                    # get_key_columns returns [(col, ref_table, ref_col), ...]
                    key_columns = self.connection.introspection.get_key_columns(cursor, table)
                    for _, ref_table, _ in key_columns:
                        if ref_table in tables_set and ref_table != table:
                            # table references ref_table.
                            # table must be deleted BEFORE ref_table.
                            # So ref_table depends on table.
                            sorter.add(ref_table, table)
                except Exception:
                    # Fallback or ignore if table introspection fails
                    pass

        try:
            sorted_tables = list(sorter.static_order())
            # static_order returns items in topological order.
            # If we add(B, A): A comes before B.
            # So 'table' (child) comes before 'ref_table' (parent).
            # This is exactly what we want: DELETE child, THEN DELETE parent.
        except Exception:
            # Cycle detected or parse error, fallback to original order
